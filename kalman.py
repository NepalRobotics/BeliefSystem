...
class Kalman:
  ...
  def __init__(self, position, velocity, lob, transmitter_position):
    ...
    self.__transmitter_position = transmitter_position
    ...
    state_size = self.__state.shape[0]
    # Default observations.
    self.__observations = np.copy(self.state)
    ...
    self.__state_covariances = np.zeros((state_size, state_size))
  ...
  def __transition_function(self, current_state):
    ...
    new_state[4] = math.atan2(self.__transmitter_position[1] - current_state[1],
                              self.__transmitter_position[0] - current_state[0])
  ...
  def set_observations(self, position, velocity, lob):
    self.__observations = np.array([position[0], position[1], velocity[0],
                                  velocity[1], lob])
